     ...
     liberror_error_t **error )
{
	 ...
	 * a directory separator
	 */
	if( path_type == LIBBFIO_PATH_TYPE_ABSOLUTE )
	{
		*full_path_size = 1;
	}
	/* If the path is relative
	 ...
	 * a directory separator, if necessary
	 */
	else if( path_type == LIBBFIO_PATH_TYPE_RELATIVE )
	{
     ...
     liberror_error_t **error )
{
	 ...
	 && ( path_type != LIBBFIO_PATH_TYPE_UNC ) )
	{
		...
		if( volume_name != NULL )
		{
			if( libbfio_path_get_current_working_directory_wide(
			     &current_working_directory,
			     &current_working_directory_size,
			     error ) != 1 )
			{
...
/* TODO add pre Windows XP support */
	...
	*full_path_size = 4;
	...
	if( current_directory_split_string != NULL )
	{
		if( libbfio_wide_split_string_get_number_of_segments(
		     current_directory_split_string,
		     &current_directory_number_of_segments,
		     error ) != 1 )
		{
	...
	for( path_segment_index = 0;
	     ...
	     path_segment_index++ )
	{
		if( libbfio_wide_split_string_get_segment_by_index(
		     path_split_string,
		     path_segment_index,
		     &path_string_segment,
		     &path_string_segment_size,
		 ...
		 && ( path_string_segment[ 1 ] == (wchar_t) '.' ) )
		{
			 ...
			 && ( last_used_path_segment_index == -1 ) )
			{
				...
				*full_path_size -= current_directory_string_segment_size;
				...
				if( libbfio_wide_split_string_set_segment_by_index(
				     current_directory_split_string,
				     current_directory_segment_index,
				     NULL,
				     0,
	...
	*full_path = libcstring_wide_string_allocate(
	...
	if( libbfio_wide_split_string_free(
	     &path_split_string,
	     error ) != 1 )
	{
		liberror_error_set(
...
on_error:
	...
	*full_path_size = 0;
	...
	if( path_split_string != NULL )
	{
		libbfio_wide_split_string_free(
		 &path_split_string,
		 NULL );
	}
	if( current_directory_split_string != NULL )
